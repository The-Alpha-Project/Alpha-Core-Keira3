...
describe('MysqlQueryService', () => {
  ...
  describe('get helpers', () => {
    ...
    for (const test of [
      ...
      { name: 'getItemByStartQuest', query: `SELECT entry AS v FROM item_template WHERE start_quest = ${id}` },
      { name: 'getItemNameByStartQuest', query: `SELECT name AS v FROM item_template WHERE start_quest = ${id}` },
